...
Called by the agent after completing each step to verify correctness before moving forward. Opt-in — only fires when explicitly called via `/gate` or `on-step-complete`.
...
1. Accept inputs: `test_command` (string), `verification_type` (exit-code | output-contains | regex-match), `expected_result` (string), optional `agent` (defaults to calling agent), optional `tags` (array).
...
4. Execute `test_command` in a subprocess with a 30000ms timeout. Capture stdout, stderr, exit code, and elapsed duration.
...
8. **If failed** → show stdout/stderr to agent. If `retries_used < 3` (max retries): increment retry count, prompt agent to fix and re-invoke. If retries exhausted: return `{ "passed": false, "error": "Max retries reached" }`. Agent is blocked.
