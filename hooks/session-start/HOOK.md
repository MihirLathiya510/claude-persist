...
4. Invoke `tpl-claude-plugin:sqlite-init` to initialize or verify the database. If sqlite-init detects pending migrations, surface the migration prompt to the user before proceeding.
...
6. Invoke `claude-persist:context-builder` with operation `build`: read current state and generate the context block. If at least one state field is non-empty, inject the context block into session state as `active_context`. If all fields are empty, skip injection silently — no noise for fresh installs.
7. Invoke `tpl-claude-plugin:anatomy-indexer` to build or refresh `PROJECT_MAP.json` (skipped if index is < 1 hour old). Agents use this map to avoid redundant file reads.
